...
class DtmfDetectorStream(object):
	...
	def post_testing(self):
		...
		#characters in a row
		#with no more than
		MAX_GAP = 0.1000 #changed to 100ms because after test I found this is the longest "distinct" issue
		...
		if self.r[row] < 4.0e5:
			...
			if see_digit:
				...
				#stores the character found, and the time in the file in seconds in which the file was found
                                #Caleb Begly: The code for the rest of this function was added (and is based on the cleanup function) to support the signalling and processing of stream information.
				...
				self.previousTime = time #Now this sample is the previous time
			
				#print "curr char:", currentChar, ", last char: ", self.lastChar, ", time delta:", timeDelta #for debugging
				...
				if self.lastChar == currentChar:
					...
					if(self.glitch == True): #We already gave them a chance
						self.glitch = False
						if(self.currentCount >= MIN_CONSECUTIVE):
							self.signalCharacterRecieved(self.lastChar) #We have moved on to the next character. Signal the end of the recieve
						self.lastChar = currentChar
						self.currentCount = 1
						return
				...
				if timeDelta > MAX_GAP:
					...
					if (self.currentCount - 1) >= MIN_CONSECUTIVE:
						self.signalCharacterRecieved(currentChar) #The gap was large enough, so it is a new keypress
					self.currentCount = 1
				self.lastChar = currentChar
